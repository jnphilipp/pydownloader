...
FILES := $(shell find pydownloader/* -type f ! -path "**/__pycache__")
...
clean:
	$(Q)rm -rf ./build
	$(Q)rm -rf ./dist
	$(Q)rm -rf ./pydownloader.egg-info
	$(Q)find pydownloader -depth -name __pycache__ -exec rm -rf {} \;


install: build/copyright build/changelog.Debian.gz
	$(Q)python -m installer --destdir="${DEST_DIR}" dist/*.whl

	$(Q)install -Dm 0644 build/changelog.Debian.gz "${DEST_DIR}${DOC_DIR}"/python-pydownloader/changelog.Debian.gz
	$(Q)install -Dm 0644 build/copyright "${DEST_DIR}${DOC_DIR}"/python-pydownloader/copyright

	@echo "python-pydownloader install completed."
...
uninstall:
	...
	$(Q)rm -r "${DEST_DIR}${DOC_DIR}"/python-pydownloader
	@echo "python-pydownloader uninstall completed."
...
build/copyright: build
	$(Q)echo "Upstream-Name: pydownloader" > build/copyright
	$(Q)echo "Source: https://github.com/jnphilipp/pydownloader" >> build/copyright
...
build/changelog.Debian.gz: build
	$(Q)( \
		...
		for ((i=$${#TAGS[@]};i>=0;i--)); do \
			if [ $$i -eq 0 ]; then \
				echo -e "pydownloader ($${TAGS[$$i]}) unstable; urgency=medium" >> build/changelog; \
				...
				echo -e "pydownloader ($${TAGS[$$i-1]}-$$(git log -n 1 --format='%h')) unstable; urgency=medium" >> build/changelog; \
